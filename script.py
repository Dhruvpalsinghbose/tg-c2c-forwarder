...
def load_config():
    ...
    logging.info(f"Caption settings: USE_ORIGINAL_CAPTIONS={config['USE_ORIGINAL_CAPTIONS']}, " 
                 f"USE_FILENAME_AS_CAPTION={config['USE_FILENAME_AS_CAPTION']}, "
                 f"APPLY_CUSTOM_CAPTION={config['APPLY_CUSTOM_CAPTION']}")
...
ALERT_INTERVAL = 600
MAX_FLOOD_WAIT = 1800
CONCURRENT_LIMIT = 1  # Set to 1 for strict sequential processing
...
def format_caption(filename=None, duration=None, original_caption=None):
    ...
    logging.info(f"Config values - USE_ORIGINAL_CAPTIONS: {config.get('USE_ORIGINAL_CAPTIONS')}, USE_FILENAME_AS_CAPTION: {config.get('USE_FILENAME_AS_CAPTION')}, APPLY_CUSTOM_CAPTION: {config.get('APPLY_CUSTOM_CAPTION')}")
    ...
    # Convert environment variables to proper boolean values
    use_original = config.get("USE_ORIGINAL_CAPTIONS", False)
    if isinstance(use_original, str):
        use_original = use_original.lower() in ("true", "1", "yes")
        
    use_filename = config.get("USE_FILENAME_AS_CAPTION", False)
    if isinstance(use_filename, str):
        use_filename = use_filename.lower() in ("true", "1", "yes")
    
    apply_custom = config.get("APPLY_CUSTOM_CAPTION", False)
    if isinstance(apply_custom, str):
        apply_custom = apply_custom.lower() in ("true", "1", "yes")
    
    logging.info(f"Boolean flags - use_original: {use_original}, use_filename: {use_filename}, apply_custom: {apply_custom}")
